...
const createPlayersDetailsForm = () => {
    ...
    for (let index = 0; index < numPlayers.value; index++) {
        ...
        div.classList.add("player_details",`player${index+1}`)
        
        const nameInp = document.createElement("input")
        nameInp.classList.add("player_name_inp","setup-inp")
        nameInp.id = `player_name_${index + 1}`; // הוספת ID לשדה הקלט

        const nameLbl = document.createElement("label")
        nameLbl.classList.add("player_name_lbl","setup-lbl")
        nameLbl.textContent = "Name:"
        nameLbl.setAttribute("for", nameInp.id); // הוספת תכונה for ל-label

        const colorInp = document.createElement("input")
        colorInp.classList.add("player_color_inp","setup-inp")
        colorInp.id = `player_color_${index + 1}`; // הוספת ID לשדה הקלט
        colorInp.setAttribute("type", "color"); // הוספת תכונה for ל-label

        const colorLbl = document.createElement("label")
        colorLbl.classList.add("player_color_lbl","setup-lbl")
